...
try:
    ...
    def ocr_page_sync(page, api_key=None): return ""
    async def ocr_page_async(page, api_key=None): return ""
    def merge_ocr_into_elements(elements, ocr_text): return elements
...
async def _scrape_single_page_async(context, url_norm: str, depth: int, base_domain: str,
    ...
    try:
        ...
        # OCR via Google Vision API (async)
        ...
        # Merge OCR results into elements (deduped)
        if ocr_text:
            elements = merge_ocr_into_elements(elements, ocr_text)
        content_hash = hashlib.sha256(body_text.encode("utf-8")).hexdigest()
        tech_stack = merge_retire_into_tech_stack(
