...
const ChatBotNew = ({ onNavigate }) => {
  ...
  const proceedToReport = async (rcaSummaryText, crawlPoints) => {
    if (userEmail || otpVerified) {
      ...
      const authMsg = {
        id: getNextMessageId(),
        text: `Your diagnostic is ready.\n\nVerify your identity to unlock your **AI Growth Playbook**.`,
        sender: 'bot',
        timestamp: new Date(),
        showAuthGate: true,
      };
              ...
              {messages.map((message) => (
                <div key={message.id} className={`message ${message.sender === 'user' ? 'user' : 'bot'}`}>
                  ...
                  <div className="message-content">
                    ...
                    {/* Auth Gate — in-chat (Google + OTP + Skip) */}
                    {message.showAuthGate && (
                      <div className="auth-gate-card">
                        {(userEmail || otpVerified) ? (
                          <div className="auth-gate-signed-in">
                            ...
                            <span>{userEmail ? <>Signed in as <strong>{userName}</strong></> : 'Phone verified ✅'}</span>
                          </div>
                        ) : (
                          <>
                            {/* OTP Verify */}
                            <button onClick={() => { pendingAuthActionRef.current = 'recommendations'; setShowAuthModal(true); }} className="auth-gate-otp-btn">
