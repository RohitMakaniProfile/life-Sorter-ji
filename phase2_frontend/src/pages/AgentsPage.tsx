...
// @ts-ignore – emoji-mart data bundle has no typed export
...
export default function AgentsPage() {
  ...
  return (
    <div className="h-full overflow-y-auto p-6 sm:p-8">
      ...
      {/* Emoji picker modal (opens when clicking icon in edit dialog) */}
      {editingId && emojiPickerOpen && (() => {
        const agent = agents.find((a) => a.id === editingId);
        if (!agent) return null;
        return (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-5 max-w-sm w-full">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span className="text-sm text-slate-600">Choose icon for</span>
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden">
                <Picker
                  ...
                  onEmojiSelect={(emoji: any) => {
                    updateAgent({ ...agent, emoji: emoji.native });
