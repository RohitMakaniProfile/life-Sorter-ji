...
@register_task_stream("playbook/onboarding-generate-v2")
async def onboarding_playbook_generate_v2_task(send, payload: dict[str, Any]) -> dict[str, Any]:
    """
    Single-prompt playbook generator (v2).

    Same input payload and output format as playbook/onboarding-generate (v1).
    ...
    try:
        ...
        logger.info("playbook/onboarding-generate-v2 done", onboarding_id=onboarding_id, playbook_run_id=str(playbook_run_id))
        return result_payload

    except Exception as exc:
        error_message = str(exc)
        logger.error("playbook/onboarding-generate-v2 failed", onboarding_id=onboarding_id, error=error_message)
        try:
            async with pool.acquire() as conn:
                if playbook_run_id:
            ...
            logger.error("playbook/onboarding-generate-v2 error update failed", error=str(db_err))
        raise