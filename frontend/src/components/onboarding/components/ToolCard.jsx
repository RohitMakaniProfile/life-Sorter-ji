 ...
 */
export default function ToolCard({ name, rating, description, bullets = [], tag, url, className = '' }) {
  return (
    <a
      href={url}
      target="_blank"
      rel="noopener noreferrer"
      className={`flex min-w-[260px] max-w-[320px] shrink-0 flex-col rounded-[14px] border border-[#b3b3b3] bg-[#161616] font-sans text-white no-underline shadow-[0_2px_8px_rgba(0,0,0,0.3)] transition-all duration-[650ms] hover:-translate-y-[3px] hover:border-white/[0.22] hover:shadow-[0_4px_16px_rgba(0,0,0,0.4)] ${className}`}
    >
