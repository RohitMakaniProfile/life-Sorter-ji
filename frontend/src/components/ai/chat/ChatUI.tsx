...
const mdComponents: React.ComponentProps<typeof ReactMarkdown>['components'] = {
  a: ({ href, children }) => (
    <a href={href} target="_blank" rel="noopener noreferrer" className="underline text-violet-300 hover:text-violet-200">{children}</a>
  ),
  p: ({ children }) => <p className="mb-1 last:mb-0 text-slate-100">{children}</p>,
  ul: ({ children }) => <ul className="list-disc pl-4 mb-1 text-slate-100">{children}</ul>,
  ol: ({ children }) => <ol className="list-decimal pl-4 mb-1 text-slate-100">{children}</ol>,
  li: ({ children }) => <li className="mb-0.5 text-slate-100">{children}</li>,
  strong: ({ children }) => <strong className="font-semibold text-slate-100">{children}</strong>,
  code: ({ children }) => <code className="rounded px-1 text-xs font-mono bg-slate-800 text-violet-300">{children}</code>,
  pre: ({ children }) => (
  ...
  thead: ({ children }) => <thead className="bg-violet-500/15 text-slate-100">{children}</thead>,
  tbody: ({ children }) => <tbody className="divide-y divide-slate-800">{children}</tbody>,
  tr: ({ children }) => <tr className="hover:bg-slate-800 transition-colors">{children}</tr>,
  th: ({ children }) => (
    <th className="px-3 py-2 text-left font-semibold text-slate-100 text-[11px] uppercase tracking-wide whitespace-nowrap border-b border-slate-700">{children}</th>
  ),
  td: ({ children }) => <td className="px-3 py-2 text-slate-200 align-top">{children}</td>,
  h1: ({ children }) => <h1 className="text-xl font-bold text-slate-100 mt-4 mb-2 pb-1 border-b border-slate-700">{children}</h1>,
  h2: ({ children }) => <h2 className="text-base font-bold text-slate-100 mt-4 mb-1.5">{children}</h2>,
...
function PlanMessage({
  ...
  return (
    <div className="bg-slate-900 text-slate-100 border border-slate-700 rounded-2xl rounded-tl-none shadow-sm overflow-hidden w-full">
      <div className="px-5 pt-3.5 pb-2 flex items-start justify-between gap-3">
        <div className="text-sm font-semibold text-slate-100">Todo (edit then start working)</div>
      ...
      <div className="px-5 pb-4">
        ...
        <div className="rounded-xl border border-slate-700 bg-slate-900 p-3 overflow-y-auto">
        ...
        {editing && canEdit && !started && (
          <div className="mt-3">
            ...
            <textarea
              ...
              rows={14}
              className="w-full resize-y rounded-xl border border-slate-700 p-3 text-sm font-mono bg-slate-800 text-slate-100 focus:outline-none focus:ring-2 focus:ring-violet-500"
            />
          </div>
        )}
...
function AssistantMessage({
  ...
  return (
    <div
      className={
        m.role === 'assistant'
          ? 'bg-slate-900 text-slate-100 border border-slate-700 rounded-2xl rounded-tl-none shadow-sm overflow-hidden'
          : ''
      }
    >
        ...
        <>
          ...
          {isReport && (
            <div className="sticky bottom-0 z-10 bg-slate-900 border-t border-slate-700">
              ...
              <button
                onClick={() => setExpanded((v) => !v)}
                className="w-full flex items-center justify-center gap-2 px-5 py-2.5 text-xs font-medium text-violet-300 hover:bg-slate-800 transition-colors"
              >
                {expanded ? (
                  <>
              ...
              <div className="flex items-center justify-between gap-3 px-5 py-2.5 border-t border-slate-700">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-600 hover:bg-slate-800 active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed text-slate-200 text-xs font-semibold rounded-lg transition-all"
                  >
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
...
export default function ChatUI({
  ...
  const canUseContextPanel = !isProd || getIsSuperAdmin();
  ...
  return (
    <div className="flex h-full w-full bg-slate-950 overflow-hidden">
      ...
      {/* Header */}
      <header className="px-6 py-4 border-b border-slate-800 bg-slate-950 flex items-center justify-between flex-shrink-0">
        <div>
        ...
        <div className="flex items-center gap-2">
          {canUseContextPanel && (
            <button
              ...
              className="px-3 py-1.5 text-xs font-semibold rounded-lg border border-slate-700 text-slate-200 hover:bg-slate-800"
      ...
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto bg-slate-950 p-4 sm:p-8 flex flex-col min-h-0"
      >
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center min-h-[200px] text-center space-y-4 opacity-70">
            ...
            <div className="space-y-2">
              <p className="text-slate-100 font-semibold text-lg">AI Agent</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2 max-w-lg w-full">
              {emptyStateSuggestions.map((s) => (
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-sm text-slate-300 hover:border-violet-500/60 hover:bg-slate-800 hover:text-violet-300 transition-all text-left cursor-pointer"
                >
        ...
        {loading && messages[messages.length - 1]?.role !== 'assistant' && (
          <div className="flex justify-start">
            ...
            <div className="px-5 py-4 bg-slate-900 border border-slate-700 rounded-2xl rounded-tl-none shadow-sm flex items-center gap-1">
              <div className="w-1.5 h-1.5 bg-violet-400 rounded-full animate-bounce [animation-delay:-0.3s]" />
              <div className="w-1.5 h-1.5 bg-violet-400 rounded-full animate-bounce [animation-delay:-0.15s]" />
              <div className="w-1.5 h-1.5 bg-violet-400 rounded-full animate-bounce" />
            </div>
      ...
      {/* Input */}
      <div className="p-4 sm:p-6 bg-slate-950 border-t border-slate-800 flex-shrink-0">
        <form onSubmit={handleSubmit} className="relative max-w-5xl mx-auto flex items-end gap-3">
          <textarea
            ...
            className="w-full resize-none bg-slate-900 focus:bg-slate-900 border border-slate-700 focus:ring-2 focus:ring-violet-500 rounded-2xl px-5 py-3.5 pr-14 text-slate-100 transition-all placeholder:text-slate-500 shadow-inner outline-none"
          />
          <button
            type="submit"
            disabled={disabled || loading}
        ...
        <p className="text-[10px] text-center text-slate-600 mt-3 font-medium uppercase tracking-wider">
