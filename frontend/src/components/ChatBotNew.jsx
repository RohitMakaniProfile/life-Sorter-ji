...
const parsePlaybookSteps = (text) => {
  ...
  const normalized = text
    ...
    // 1. The "Title" Word  OR  1. The "Title Word"  — capture full rest of line
    .replace(/^(?:#{0,3}\s*)?(\d{1,2})\.\s+((?:The\s+)?[""\u201C][^"\n]*[""\u201D]?[^\n]*|The\s+[A-Z][^\n]+)\s*$/gm, '___STEP_$1___$2');

  const blocks = normalized.split(/^___STEP_(\d+)___(.*)$/m);

  for (let i = 1; i < blocks.length; i += 3) {
    const num = parseInt(blocks[i]);
    // Strip markdown bold and smart/regular quotes from title
    const title = blocks[i + 1]?.replace(/\*\*/g, '').replace(/["\u201C\u201D]/g, '').trim() || '';
    ...
    steps.push({ num, title, subsections, rawText: `${title}\n${body}` });
  }
...
const STEP_TYPE_STYLES = {
  research:  { label: 'Research',  color: '#1d4ed8', bg: '#eff6ff',  border: '#bfdbfe' },
  content:   { label: 'Content',   color: '#7c3aed', bg: '#f5f3ff',  border: '#ddd6fe' },
  outreach:  { label: 'Outreach',  color: '#065f46', bg: '#f0fdf4',  border: '#bbf7d0' },
  execute:   { label: 'Executed',  color: '#92400e', bg: '#fffbeb',  border: '#fde68a' },
  strategy:  { label: 'Strategy',  color: '#92400e', bg: '#fffbeb',  border: '#fde68a' },
};

// ── Renders structured playbook steps ──
const PlaybookStepsRenderer = ({ content, sessionId }) => {
  const { steps, checklist } = parsePlaybookSteps(content);
  const [stepResults, setStepResults] = useState({});
  const [sheetExports, setSheetExports] = useState({});

  const runStep = async (step) => {
    if (!sessionId) return;
    setStepResults(prev => ({ ...prev, [step.num]: { loading: true } }));
    try {
      const res = await fetch('/api/v1/execute/step', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          session_id: sessionId,
          step_number: step.num,
          step_text: step.rawText,
        }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.detail || 'Execution failed');
      setStepResults(prev => ({ ...prev, [step.num]: { loading: false, ...data } }));
    } catch (err) {
      setStepResults(prev => ({ ...prev, [step.num]: { loading: false, error: err.message } }));
    }
  };

  const exportToSheets = async (step) => {
    const res = stepResults[step.num];
    if (!res?.output) return;
    setSheetExports(prev => ({ ...prev, [step.num]: { loading: true } }));
    try {
      const resp = await fetch('/api/v1/execute/export-sheet', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          session_id: sessionId,
          step_number: step.num,
          step_type: res.step_type || 'general',
          output: res.output,
          sources: res.sources || [],
      ...
      setSheetExports(prev => ({ ...prev, [step.num]: { loading: false, url: data.sheet_url } }));
    } catch (err) {
      setSheetExports(prev => ({ ...prev, [step.num]: { loading: false, error: err.message } }));
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      {steps.map((step, si) => {
        const res = stepResults[step.num];
        const typeStyle = res?.step_type ? STEP_TYPE_STYLES[res.step_type] : null;
        return (
        <div key={si} style={{
          background: '#fff',
          ...
          <div style={{
            ...
            <div style={{ fontSize: '0.95rem', fontWeight: 800, color: '#1e1b4b', letterSpacing: '-0.02em', lineHeight: 1.3, flex: 1 }}>
              {step.title}
            </div>
            {/* Run Step button */}
            {sessionId && (
              <button
                ...
                style={{
                  ...
                  borderRadius: '8px',
                  border: 'none',
                  background: res?.output ? '#f0fdf4' : '#7c3aed',
                  color: res?.output ? '#065f46' : '#fff',
                  fontSize: '0.72rem', fontWeight: 700,
                  cursor: res?.loading ? 'not-allowed' : 'pointer',
                  opacity: res?.loading ? 0.7 : 1,
                  transition: 'all 0.2s ease',
                  whiteSpace: 'nowrap',
                }}
              >
                {res?.loading ? '⏳ Running...' : res?.output ? '✓ Done' : '▶ Run Step'}
              </button>
            )}
          </div>
          ...
          <div style={{ padding: '0.85rem', display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
            ...
            {res?.error && (
              <div style={{ borderRadius: '10px', background: '#fef2f2', border: '1px solid #fecaca', padding: '0.8rem 1rem' }}>
                <div style={{ fontSize: '0.72rem', fontWeight: 800, color: '#dc2626', marginBottom: '0.3rem' }}>EXECUTION ERROR</div>
                <div style={{ fontSize: '0.82rem', color: '#7f1d1d' }}>{res.error}</div>
              </div>
            )}
            {res?.output && (
              <div style={{
                borderRadius: '10px',
                background: typeStyle?.bg || '#f0fdf4',
                border: `1px solid ${typeStyle?.border || '#bbf7d0'}`,
                padding: '0.9rem 1rem',
              }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.6rem' }}>
                  <div style={{
                    display: 'inline-flex', alignItems: 'center', gap: '0.35rem',
                    fontSize: '0.7rem', fontWeight: 800, color: typeStyle?.color || '#065f46',
                    letterSpacing: '0.05em', textTransform: 'uppercase',
                  }}>
                    <span>🤖</span> AI Agent Output — {typeStyle?.label || res.step_type}
                  </div>
                  <span style={{ fontSize: '0.65rem', color: '#9ca3af' }}>{res.latency_ms ? `${(res.latency_ms/1000).toFixed(1)}s` : ''}</span>
                </div>
                <div className="playbook-markdown" style={{ fontSize: '0.845rem', color: '#374151', lineHeight: 1.75 }}>
                  <ReactMarkdown remarkPlugins={[remarkGfm]}>{res.output}</ReactMarkdown>
                </div>
                {res.sources?.length > 0 && (
                  <div style={{ marginTop: '0.75rem', paddingTop: '0.6rem', borderTop: '1px solid #d1fae5' }}>
                    <div style={{ fontSize: '0.68rem', fontWeight: 800, color: '#065f46', marginBottom: '0.4rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Sources</div>
                    {res.sources.slice(0, 4).map((s, i) => (
                      <div key={i} style={{ fontSize: '0.75rem', color: '#1d4ed8', marginBottom: '0.2rem' }}>
                        <a href={s.link} target="_blank" rel="noopener noreferrer" style={{ color: '#1d4ed8' }}>{s.title || s.link}</a>
                      </div>
                    ))}
                  </div>
                )}
                {/* Export to Google Sheets */}
                <div style={{ marginTop: '0.75rem', paddingTop: '0.6rem', borderTop: `1px solid ${typeStyle?.border || '#d1fae5'}`, display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                  {sheetExports[step.num]?.url ? (
                    <a
                      href={sheetExports[step.num].url}
                      target="_blank"
                    ...
                    <button
                      onClick={() => exportToSheets(step)}
                      disabled={sheetExports[step.num]?.loading}
                      style={{
                        display: 'inline-flex', alignItems: 'center', gap: '0.35rem',
                        padding: '0.35rem 0.75rem', borderRadius: '7px',
                        background: '#fff', border: '1px solid #d1d5db',
                        color: '#374151', fontSize: '0.72rem', fontWeight: 700,
                        cursor: sheetExports[step.num]?.loading ? 'not-allowed' : 'pointer',
                        opacity: sheetExports[step.num]?.loading ? 0.6 : 1,
                      }}
                    >
                      {sheetExports[step.num]?.loading ? 'Exporting...' : 'Export to Sheets'}
                    </button>
                  )}
                  {sheetExports[step.num]?.error && (
                    <span style={{ fontSize: '0.7rem', color: '#dc2626' }}>{sheetExports[step.num].error}</span>
                  )}
                </div>
              </div>
            )}
...
const PlaybookCollapsible = ({ title, icon, color, bg, borderColor, content, isPlaybookSteps, sessionId }) => {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div ref={contentRef} style={{ borderTop: `1px solid ${borderColor}`, padding: '1.25rem 1.5rem' }}>
          {isPlaybookSteps ? (
            <PlaybookStepsRenderer content={content} sessionId={sessionId} />
          ) : (
            <div className="playbook-markdown" style={{ fontSize: '0.875rem', color: '#1e293b', lineHeight: 1.8 }}>
              <ReactMarkdown remarkPlugins={[remarkGfm]}>{formatSectionMarkdown(content)}</ReactMarkdown>
            </div>
